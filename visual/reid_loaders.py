...
class ReIDLoaders:
    ...
    def __init__(self, config):
        ...
        self.s_train_dataset = config.s_train_dataset
        self.t_train_dataset = config.t_train_dataset
        self.test_dataset = config.test_dataset
        for a_train_dataset in self.s_train_dataset:
            assert a_train_dataset in self.datasets
        for a_train_dataset in self.t_train_dataset:
            assert a_train_dataset in self.datasets
    ...
    def _load(self):
        ...
        '''init train dataset'''
        #获取源域samples
        s_train_samples = self._get_train_samples(self.s_train_dataset)
        #获取目标域samples
        t_train_samples = self._get_train_samples(self.t_train_dataset)
        #源域 目标域 迭代器
        self.s_train_iter = self._get_uniform_iter(s_train_samples, self.transform_train, self.p, self.k)
        self.t_train_iter = self._get_random_iter(t_train_samples, self.transform_train, self.p*self.k)
        '''init test dataset'''
