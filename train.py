...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--config_file", default="configs/BPDA.yml", help="path to config file", type=str
    )
    ...
    model, class_net, class_net2 = make_model(cfg, num_class=num_classes,
                                              num_class_t=c_pids, camera_num=camera_num, view_num=view_num)

    loss_func, ide_creiteron = make_loss(cfg, num_classes=num_classes)

    optimizer, class_optimizer, class2_optimizer = make_optimizer(cfg, model, class_net, class_net2)

    scheduler, class_scheduler, class2_scheduler = create_scheduler(cfg, optimizer, class_optimizer, class2_optimizer)
    ...
    do_train(
        ...
        scheduler,
        loss_func,
        target_num_query, args.local_rank, class_optimizer, class2_optimizer, class_scheduler, class2_scheduler,
        target_train_loader, ide_creiteron, loss_classifier, t_loader
    )
